...
//           <div className="mb-6">
//             <div className="flex bg-white dark:bg-slate-900 pl-4 pt-4 items-center justify-between pb-2">
//               <button
...
function App() {
  ...
  const { user } = useAuth();
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { isQuizMode } = useQuizMode(); // Get isQuizMode from context
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
      ...
      <main className={`${sidebarOpen && !isQuizMode ? 'lg:ml-64' : 'lg:ml-0'} lg:p-8 transition-all duration-300`}>
        <div className="max-w-7xl mx-auto">
      ...
      {!isQuizMode && <BottomNavigationBar />}
    </div>
  );
}
