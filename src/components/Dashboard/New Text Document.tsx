...
export class MentorAnalyzer {
  ...
  private static async calculateComprehensiveMetrics(data: any): Promise<ComprehensiveUserProfile> {
    ...
    const studyPatterns = this.analyzeStudyPatterns(studySessions, learningAnalytics);
  ...
  private static analyzeStudyPatterns(studySessions: any[], learningAnalytics: any[]) {
    // Analyze time slots
  ...
  private static calculateLearningMetrics(
    ...
    const conceptsLearnedPerHour = totalStudyHours > 0 ? masteredTopics / totalStudyHours : 0;
  ...
  private static calculateRetentionRate(quizAttempts: any[]): number {
    ...
    const topicQuizzes: Record<string, any[]> = {};
    quizAttempts.forEach(quiz => {
      if (!topicQuizzes[quiz.topic]) {
        topicQuizzes[quiz.topic] = [];
      }
      topicQuizzes[quiz.topic].push(quiz);
    });
  ...
  private static analyzeScheduleAdherence(
    ...
    // Estimate missed sessions (simplified)
    const expectedSessions = Math.floor(plannedDailyHours * 7); // Weekly expected
    const actualWeeklySessions = studySessions.filter(s => {
      const sessionDate = new Date(s.created_at);
      const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      return sessionDate >= weekAgo;
    }).length;
    const missedSessions = Math.max(0, expectedSessions - actualWeeklySessions);
