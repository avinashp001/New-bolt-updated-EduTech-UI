...
const WeeklyPlanTracker: React.FC = () => {
  ...
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [testResult, setTestResult] = useState<any>(null);
  const [showTestInterface, setShowTestInterface] = useState(false); // Controls quiz display
  const [questions, setQuestions] = useState<any[]>([]);
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [userAnswers, setUserAnswers] = useState<number[]>([]);
  const [testStartTime, setTestStartTime] = useState<Date | null>(null);
  ...
  useEffect(() => {
    if (showTestInterface) {
      setQuizMode(true); // Enter quiz mode when test interface is shown
    } else {
      setQuizMode(false); // Exit quiz mode otherwise
    }
  ...
  const finishWeeklyTest = async () => {
    ...
    try {
      ...
      setShowTestInterface(false); // Hide the quiz interface after finishing
  ...
  const resetTest = () => {
    setShowTestInterface(false); // Hide the quiz interface
    setQuestions([]);
    setUserAnswers([]);
    setCurrentQuestionIndex(0);
    setTestResult(null);
