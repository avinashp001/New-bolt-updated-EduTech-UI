...
const FAQPage: React.FC = () => {
  ...
  // Create a ref to store references to FAQ answer elements
  ...
  const toggleFAQ = (faqId: string) => {
    setOpenFAQ(prevOpenFAQ => {
      const newOpenFAQ = prevOpenFAQ === faqId ? null : faqId;
      if (newOpenFAQ && faqRefs.current[newOpenFAQ]) {
        // Use a timeout to allow the DOM to update after state change (expansion animation)
        setTimeout(() => {
          faqRefs.current[newOpenFAQ]?.scrollIntoView({ behavior: 'smooth', block: 'start' });
          faqRefs.current[newOpenFAQ]?.focus(); // Focus the element for accessibility
        }, 100); // Small delay to ensure element is rendered and expanded
      }
      return newOpenFAQ;
    });
  };
          ...
          <div className="space-y-4">
            {filteredFAQs.map((faq, index) => (
              <div key={faq.id} className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden hover:shadow-xl transition-all duration-300">
                ...
                {openFAQ === faq.id && (
                  <div 
                    ref={el => faqRefs.current[faq.id] = el} // Assign ref here
